...
async function fetchTokenInfo(contract) {
  try {
    ...
    console.log(`[TrendingBot] Fetched info for contract ${contract}: ${info ? 'Success' : 'No data'}`);
    return info;
  } catch (e) {
...
async function computeTrending() {
  ...
  for (const p of pairs) {
    ...
    if (!info || !info.transactions?.length) {
      console.log(`[TrendingBot] Skipping ${p.token0.symbol}/${p.token1.symbol}: No info or transactions`);
      continue;
    }
    ...
    console.log(`[TrendingBot] Found ${recentTxs.length} recent transactions for ${p.token0.symbol}/${p.token1.symbol}`);
